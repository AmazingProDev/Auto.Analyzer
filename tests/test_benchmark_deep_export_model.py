...
class BenchmarkDeepExportModelTests(unittest.TestCase):
    ...
    def test_export_model_carries_simplified_five_columns(self):
        dataset = self._dataset_fixture()
        deep = {
            ...
            "actionPlan": [
                {
                    "priority": "P1",
                    "domain": "Active DL / 5G capacity layer",
                    "finding": "IAM DL session is LTE-dominated.",
                    "action": "Analyze EN-DC addition/release during the DL transfer.",
                    "confidence": "Confirmed symptom / Partial root cause",
                }
            ],
        }
        ...
        self.assertTrue({"priority", "domain", "finding", "action", "confidence"} <= set(row.keys()))
        self.assertEqual(row["action"], "Analyze EN-DC addition/release during the DL transfer.")
        self.assertEqual(row["confidence"], "Confirmed symptom / Partial root cause")

    def test_export_model_fills_missing_simplified_keys(self):
        dataset = self._dataset_fixture()
        deep = {
            "execSummary": {"title": "DT 1"},
            "kpiBenchmark": [],
            "actionPlan": [{"priority": "P3", "domain": "Retest governance", "finding": "Single DT."}],
        }
        model = server._benchmark_deep_export_model(dataset, deep)
        row = model["actionPlan"][0]
